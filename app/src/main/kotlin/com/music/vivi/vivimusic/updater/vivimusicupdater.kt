...
fun isNewerVersion(latestVersion: String, currentVersion: String): Boolean {
    // Our tags carry the desktop pairing (`6.0.6.8_DE-1.53.26`) and upstream's
    // carry a nightly run number (`-r2`): strip both before comparing, and keep
    // the run number as the tie-breaker for two builds of the same version.
    val latestVersionClean = mobileVersionFromTag(latestVersion)
    val currentVersionClean = mobileVersionFromTag(currentVersion)
    val latestRun = Regex("-r(\\d+)").find(latestVersionClean)?.groupValues?.get(1)?.toIntOrNull() ?: 0
    val currentRun = Regex("-r(\\d+)").find(currentVersionClean)?.groupValues?.get(1)?.toIntOrNull() ?: 0
    val latestBase = latestVersionClean.substringBefore("-r")
    val currentBase = currentVersionClean.substringBefore("-r")
        ...
        try {
            ...
            if (targetRelease != null) {
                ...
                if (shouldShow) {
                    ...
                    try {
                        val changelogUrl =
                            URL("https://github.com/${updateRepo(context)}/releases/download/$tagWithPrefix/changelog.json?t=${System.currentTimeMillis()}")
