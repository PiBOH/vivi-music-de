...
- [ ] **`G1 Young Generation, System.gc()` fires about once a minute.** It is Skiko’s own periodic explicit GC (Compose Desktop calls `System.gc()` every 30 s; see the comment in `desktop/build.gradle.kts`), already mitigated by `-XX:+ExplicitGCInvokesConcurrent`. The pauses it caused in these logs are 5-20 ms; only one 111 ms `G1 Evacuation Pause` appears (22:28:09, no audio symptom). No action unless a stall line appears next to one.
...
- [x] **The notification-permission row (done 26 Sep, 1.53.29, shown on every platform from 1.53.30).** A row on the Notifications screen: on macOS it asks for the permission and posts a notification right after, so the prompt appears and its result is visible; on Windows and Linux (no runtime permission) it is shown too and posts a notification, so the row verifies delivery everywhere instead of only where a permission exists.
